 ...
 * 几何: 2D 圆环 r_in=0.2m, r_out=0.5m, 中心在原点（外圆减内圆 → 有孔，几何非平凡）
 * 边界:
 *   内环(xy 平面内 r≈0.2) TemperatureBoundary T=T1=473K（Dirichlet）
 *   外环(r≈0.5) HeatFluxBoundary 对流换热 h=100 W/m²K, T∞=293K（Robin）
 *   其余边界默认热绝缘
 * 初值: T=293K（=T∞）
 * 材料: k=50 W/mK, ρ=7850 kg/m³, Cp=500 J/kgK（钢）
 * 研究: 瞬态 TimeDependent, t=0..3000s（保存 5 个时间点: 0,100,500,1500,3000s）
 *
 * 解析解（圆环稳态 + 贝塞尔级数，Carslaw & Jaeger §7.2 型）:
 *   稳态（径向）: T_s(r) = T∞ + A·(ln(r/a) - B), 满足内壁 Dirichlet + 外壁 Robin。
 *   瞬态:        T(r,t) = T_s(r) + Σ_n c_n·e^{−α λ_n² t}·U_n(λ_n r)
 *   其中 U_n 为 J0/Y0 组合满足边值，λ_n 为超越方程根，c_n 由初条件正交投影。
 *   本类仅导出 CSV，解析解计算在 scripts/verifications/verify_t2_ring_transient.py。
 *
 * 本机证据（COMSOL 6.2 官方示例 heating_circuit.mph 解包）:
 *   - 对流边界特征: HeatFluxBoundary, 参数 HeatFluxType='ConvectiveHeatFlux',
 *     minput_temperature_src='userdef', minput_temperature, HeatTransferCoefficientType='UserDef', h
 *   - 瞬态研究: study("std1").create("time", "TimeDependent");
 *     study("std1").feature("time").set("tlist", "range(...)")
 *   - 初值: ht.init 或 physics("ht").feature("init1").set("T", ...)
 *
 * 模块需求: Heat Transfer 模块
 * 运行: python scripts/run.py all TRingTransient <run-dir>
...
public class TRingTransient {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // 属性名: thermalconductivity/density/heatcapacity（heating_circuit.mph 证据）
        // 注意: density/heatcapacity 若引用全局参数 rho/Cp 会与 physics 特征同名
        // 参数冲突解析成 0 → 直接内联数值）
        ...
        // 按坐标选边界: 内环(r≈0.2) TemperatureBoundary, 外环(r≈0.5) HeatFluxBoundary(对流)
        ...
        model.study("std1").feature("time").set("tlist",
            "range(0,100[s],500[s]) range(500,1000,1500[s]) range(1500,1500,3000[s])");
    ...
    private static void requireEdges(String label, int[] edges) {
        if (edges == null || edges.length == 0) {
            throw new IllegalStateException(
                "edgesAtRadius: no edge matched for '" + label + "'");
        }
    }
}
