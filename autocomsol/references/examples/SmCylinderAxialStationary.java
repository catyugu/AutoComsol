...
public class SmCylinderAxialStationary {
    public static void main(String[] args) throws Exception {
        ...
        model.component(comp).physics("solid").feature("bl1").set("FperArea", new String[]{"0", "0", "sigma_z"});
    ...
    private static int[] facesAtZ(GeomInfo gi, double zt) {
        int n = gi.getNFaces(); int[] tmp = new int[n]; int c = 0;
        for (int f = 1; f <= n; f++) {
        ...
        int[] r = new int[c]; System.arraycopy(tmp,0,r,0,c); return r;
    }
    private static double[] sampleCenter(GeomInfo gi, int f, double[] pr) {
        try {
            ...
            try { double[][] pts = gi.faceX(f, new double[][]{{0.5,0.5}});
                  if (pts != null && pts.length>0) return pts[0]; } catch (Exception e2) {}
        }
        return null;
    }
}
